...
export default function Home() {
  ...
  const wordStatus = useMemo(() => {
    ...
    if (wordCount < 70)
      return { color: "text-red-400", label: `❌ Слишком мало! Меньше 70 слов = 0 баллов` };
    if (wordCount < 150)
      return { color: "text-yellow-400", label: `⚠️ Мало слов. От 70 до 149 — К7–К12 снижаются` };
    ...
    <main className="animated-gradient min-h-screen text-slate-100">
      ...
      <section className="max-w-6xl mx-auto px-6 pt-20 pb-12 text-center">
        ...
        <p className="text-lg md:text-xl text-slate-300 max-w-3xl mx-auto mb-4 leading-relaxed">
          Получите <span className="text-purple-300 font-semibold">детальный разбор</span> своего
          сочинения по всем 12 критериям всего за{" "}
          <span className="text-purple-300 font-semibold">60 секунд</span>. Найдём ошибки, поставим
          баллы и подскажем, как улучшить работу.
        </p>
      ...
      <section className="max-w-7xl mx-auto px-6 py-12">
        <div className="glass-card rounded-3xl p-8 md:p-10 shadow-2xl">
          <h2 className="font-serif text-3xl font-bold mb-6 text-white">Проверить сочинение</h2>
          ...
          <input
            ...
            placeholder="Например: проблема роли книг в жизни человека"
          ...
          >
            {loading ? "🔍 Проверяю... (это займёт ~60 сек)" : "🚀 Проверить сочинение (35 ₽)"}
          </button>
      ...
      <section className="max-w-4xl mx-auto px-6 py-8">
        <div className="glass-card rounded-2xl p-6 border-yellow-500/20">
          <p className="text-sm text-slate-400 leading-relaxed">
            <span className="text-yellow-300 font-semibold">⚠️ Важное уведомление:</span> Данный
            сервис является независимым образовательным проектом и{" "}
            ...
            гарантируют идентичного результата на реальном экзамене.
          </p>
        </div>
      </section>
      ...
      {/* МОДАЛКА ОПЛАТЫ — БЕЗ КОММЕНТАРИЯ В JSX! */}
      {showPayment && (
        <PaymentModal
          amount={35}
          essay={essay}
